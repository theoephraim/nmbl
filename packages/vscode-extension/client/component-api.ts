...
/** The result of inspecting a component's source. */
 ...
 */
export function extractImportSource(scriptText: string, componentName: string): string | null {
  const name = escapeRegExp(componentName);
  // import Name from '...'   |   import { ... Name ... } from '...'
  const re = new RegExp(
    `import\\s+(?:${name}\\b|(?:[^'";]*\\{[^}]*\\b${name}\\b[^}]*\\}))[^'";]*from\\s*(['"])([^'"]+)\\1`,
  );
...
export function extractComponentApi(source: string): ComponentApi {
  return {
 ...
 */
function extractMacro(source: string, macro: 'defineProps' | 'defineEmits'): string[] {
  ...
  if (source[i] === '<') {
    ...
    if (nameMatch) return resolveLocalType(source, nameMatch[1]);
    return [];
  }
...
function resolveLocalType(source: string, name: string): string[] {
  ...
  if (iface) {
    ...
    if (body) return extractMemberKeys(body);
  }
 ...
 * unbalanced.
 */
function balancedBody(s: string, open: number, openCh = '{', closeCh = '}'): string | null {
  let depth = 0;
 ...
 */
export function extractMemberKeys(body: string): string[] {
  const keys: string[] = [];
  let depth = 0;
  for (let i = 0; i < body.length; i++) {
    ...
    if (depth === 0 && /[A-Za-z_$]/.test(c)) {
      ...
      if (body[k] === '?') { k++; while (k < body.length && /\s/.test(body[k])) k++; }
      if (body[k] === ':') keys.push(id);
      i = j - 1;
    }
  }
  return [...new Set(keys)];
